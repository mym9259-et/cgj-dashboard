...
async def get_order_structure(
    ...
    by_brand = await _group_count(db, clauses, needs_join, "brand")
    by_model = await _group_count(db, clauses, needs_join, "model_series")

    # Price ranges (deals only)
    price_ranges = await _price_ranges(db, deal_clauses, needs_join)

    # Demographics (all leads)
    gender = await _group_count(db, clauses, needs_join, "gender")
    age_group = await _group_count(db, clauses, needs_join, "age_group")
    by_owner_type = await _group_count(db, clauses, needs_join, "owner_type")
    by_payment = await _group_count(db, clauses, needs_join, "payment_method")

    return {
        "product_distribution": {
            "by_type": by_type, "by_source": by_source, "by_years": by_years,
        },
        "brand_model_ranking": {"by_brand": by_brand, "by_model": by_model},
        "price_ranges": price_ranges,
        "demographics": {
            "gender": gender, "age_group": age_group,
            "owner_type": by_owner_type, "payment_method": by_payment,
        },
    }


async def _group_count(db, clauses, needs_join, field):
...
async def _price_ranges(db, clauses, needs_join):
    ranges = [
        (0, 2000, "0-2000"), (2000, 3000, "2000-3000"), (3000, 4000, "3000-4000"),
        (4000, 5000, "4000-5000"), (5000, 6000, "5000-6000"), (6000, 8000, "6000-8000"),
        (8000, 10000, "8000-10000"), (10000, 999999, "10000+"),
    ]
